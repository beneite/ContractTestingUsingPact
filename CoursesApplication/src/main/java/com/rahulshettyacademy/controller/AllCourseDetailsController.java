...
@Tag(
        name = "Courses API's",
        description = "We have the APIs for courses"
)
@RestController
public class AllCourseDetailsController {
    ...
    public List<AllCourseDataEntity> GetCourses() {
    ...
    public AllCourseDataEntity getBookById(@PathVariable(value = "name") String name) {
        try {
            AllCourseDataEntity lib = repository.findById(name).get();
            return lib;
        } catch (Exception e) {
